...
export default function Alert({message,type, link}:{message:string,type:string, link:string}){
...
setTimeout(()=>{
    ...
    return(
        <>
        <div id="alert-3" className={clsx("absolute t-4 flex items-center p-4 mb-4 text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400",{
            'hidden': type === 'error',
            'block': type === 'success'
        })} role="alert">
        ...
        <div className="ms-3 text-sm font-medium">
          {message} <a href={link} className="font-semibold underline hover:no-underline">example link</a>. Give it a click if you like.
        </div>
      ...
      <div id="alert-2" className={clsx("flex items-center p-4 mb-4 text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400",
        {
        "hidden": type === 'success',
        "block": type ==='error'
        }
